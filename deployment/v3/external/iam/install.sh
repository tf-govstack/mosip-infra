...
function installing_keycloak() {
  ...
  echo Installing
  helm -n $NS install $SERVICE_NAME tf-govstack/keycloak --version "7.1.18" --set image.repository=mosipid/mosip-keycloak --set image.tag=16.1.1-debian-10-r85 -f values.yaml --wait
