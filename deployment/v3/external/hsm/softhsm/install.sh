...
function installing_softhsm() {
  ...
  helm -n $NS install softhsm-kernel tf-govstack/softhsm -f values.yaml --version $CHART_VERSION --wait
  echo Installed Softhsm for Kernel

  echo Installing Softhsm for IDA
  helm -n $NS install softhsm-ida tf-govstack/softhsm -f values.yaml --version $CHART_VERSION --wait
