...
function installing_prereg() {
  ...
  helm -n $NS install prereg-gateway tf-govstack/prereg-gateway --set istio.hosts[0]=$PREREG_HOST --version $CHART_VERSION

  echo Installing prereg-captcha
  helm -n $NS install prereg-captcha tf-govstack/prereg-captcha --version $CHART_VERSION

  echo Installing prereg-application
  helm -n $NS install prereg-application tf-govstack/prereg-application --version $CHART_VERSION

  echo Installing prereg-booking
  helm -n $NS install prereg-booking tf-govstack/prereg-booking --version $CHART_VERSION

  echo Installing prereg-datasync
  helm -n $NS install prereg-datasync tf-govstack/prereg-datasync --version $CHART_VERSION

  echo Installing prereg-batchjob
  helm -n $NS install prereg-batchjob tf-govstack/prereg-batchjob --version $CHART_VERSION

  echo Installing prereg-ui
  helm -n $NS install prereg-ui tf-govstack/prereg-ui --set prereg.apiHost=$PREREG_HOST --version $CHART_VERSION
