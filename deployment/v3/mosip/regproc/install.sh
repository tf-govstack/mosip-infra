...
function installing_regproc() {
  ...
  helm -n $NS install regproc-workflow mosip/regproc-workflow --set image.repository=tfgovstackdev/registration-processor-workflow-manager-service --set image.tag=tf-develop -f workflow_values.yaml --version $CHART_VERSION
  ...
  helm -n $NS install regproc-camel mosip/regproc-camel --set image.repository=technogovstack/registration-processor-common-camel-bridge --set image.tag=release-1.2.0.1.1 --version $CHART_VERSION

  echo Installing regproc-pktserver
  helm -n $NS install regproc-pktserver mosip/regproc-pktserver --set image.repository=technogovstack/registration-processor-dmz-packet-server --set image.tag=release-1.2.0.1.1 --version $CHART_VERSION

  echo Installing group1
  helm -n $NS install regproc-group1 mosip/regproc-group1 --set image.repository=technogovstack/registration-processor-stage-group-1 --set image.tag=release-1.2.0.1.1 -f group1_values.yaml --version $CHART_VERSION

  echo Installing group2
  helm -n $NS install regproc-group2 mosip/regproc-group2 --set image.repository=technogovstack/registration-processor-stage-group-2 --set image.tag=release-1.2.0.1.1 --version $CHART_VERSION

  echo Installing group3
  helm -n $NS install regproc-group3 mosip/regproc-group3 --set image.repository=technogovstack/registration-processor-stage-group-3 --set image.tag=release-1.2.0.1.1 --version $CHART_VERSION
  ...
  helm -n $NS install regproc-group7 mosip/regproc-group7 --set image.repository=technogovstack/registration-processor-stage-group-7 --set image.tag=release-1.2.0.1.1 --version $CHART_VERSION
