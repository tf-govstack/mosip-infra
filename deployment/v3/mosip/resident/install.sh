...
function installing_resident() {
  ...
  helm -n $NS install resident tf-govstack/resident --set istio.corsPolicy.allowOrigins\[0\].prefix=https://$RESIDENT_HOST --version $CHART_VERSION

  echo Installing mimoto
  helm -n $NS install mimoto tf-govstack/mimoto --version $MIMOTO_CHART_VERSION

  echo Installing Resident UI
  helm -n $NS install resident-ui tf-govstack/resident-ui --set resident.apiHost=$API_HOST --set istio.hosts\[0\]=$RESIDENT_HOST --version $RESIDENT_UI_CHART_VERSION
